...
public:
    ...
    struct Config {
        ...
        std::string output_dir = "./dpi_output/";
        bool verbose = false;
    };
    ...
    // Wait for processing to complete — proper pipeline drain
    ...
    // Load rules from file (.ini or .json based on extension)
    ...
    // Write JSON output files to the configured output directory
...
private:
    ...
    // JSON output helpers (stdlib only, no external libs)
    void writeStatsJSON(const std::string& filepath) const;
    void writeFlowsJSON(const std::string& filepath) const;
    void writeAppStatsJSON(const std::string& filepath) const;
    
    // Atomic file write: write to temp, then rename
    static bool atomicWrite(const std::string& filepath, const std::string& content);
    
    // Rules hot-reload background loop
